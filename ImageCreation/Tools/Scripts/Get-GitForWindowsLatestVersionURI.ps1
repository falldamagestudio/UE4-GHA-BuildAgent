...
function Get-GitForWindowsLatestVersionURI {
	...
	$LatestVersionLabel = ((Invoke-WebRequest -UseBasicParsing -Uri "https://api.github.com/repos/git-for-windows/git/releases/latest" -ErrorAction Stop).Content | ConvertFrom-Json -ErrorAction Stop).tag_name

	$ParsedLatestVersionLabel = $LatestVersionLabel | Select-String -Pattern "v(\d+)\.(\d+)\.(\d+)" -ErrorAction Stop
